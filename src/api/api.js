...
export const
  listMessages = (unreadChecked, pageToken, pages, fetchedEmail, finishFetch) => new Promise(
    (resolve, reject) => {
      window.gapi.client.gmail.users.messages.list({
        userId: 'me',
        maxResults: MAX_RESULTS,
        q: unreadChecked ? 'is:unread' : '',
        pageToken,
      }).then((response) => {
        const {
          messages,
        ...
        if (messages && messages.length > 0) {
          ...
          Promise.all(promises).then(() => {
            if (nextPageToken && pagesFetched < PAGE_LIMIT) {
              listMessages(unreadChecked, nextPageToken, pagesFetched, fetchedEmail, finishFetch);
            } else {
              finishFetch();
            }
          });
